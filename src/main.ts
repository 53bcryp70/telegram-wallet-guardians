...
app.innerHTML = `
  <main class="app-shell" aria-busy="false">
    ...
    <section id="share-section" aria-labelledby="shares-heading" hidden>
      ...
      <p>Before sending a share: start a Secret Chat with that guardian and set a self-destruct timer. After the guardian saves the share offline (for example on paper), make sure the message is deleted for both sides. Sent messages in normal chats stay in your own account too — never let any account, including yours, end up holding two shares.</p>

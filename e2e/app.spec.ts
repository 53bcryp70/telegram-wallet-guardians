...
test.describe("SLIP-39 cryptographic checkpoint", () => {
  ...
  test("loads the pinned browser engine, passes vector 23, and round-trips every pair", async ({
    ...
    expect(await page.evaluate(() => navigator.clipboard.readText())).toContain(
      "one recovery share for my Wallet in Telegram DeFi Account",
    );
    expect(await page.evaluate(() => navigator.clipboard.readText())).toContain("Secret Chat");
    expect(await page.evaluate(() => navigator.clipboard.readText())).not.toContain(shareOne);
